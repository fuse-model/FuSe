...
class Model(nn.Module):
    ...
    def setup(self):
        ...
        self._proprio_model = nn.Dense(
            self._llm.embdim * self.num_proprio_tokens, name="proprio"
        )
    ...
    def embed_proprio(self, proprio, train=False):
        out = {}
        zproprio = out["proprio/zproprio"] = rearrange(
            self._proprio_model(proprio), "b (k e) -> b k e", e=self._llm.embdim
        )
        return zproprio, out

    def embed_image_and_text(
        self, image, text, proprio, *, input_mask=None, mask_ar=None, train=False
    ):
