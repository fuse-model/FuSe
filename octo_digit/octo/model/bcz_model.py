...
@struct.dataclass
class BczModel:
    module: BczModule = struct.field(pytree_node=False)
    text_processor: MuseEmbedding = struct.field(pytree_node=False)
    ...
    @partial(
        jax.jit,
        static_argnames=("train", "sample_shape"),
    )
    def sample_actions(
        self,
        observations: Data,
    ...
    def load_pretrained(
        ...
        # create model def
        module = BczModule.create(example_batch, **config["model"])
    ...
    def from_config(
        ...
        """
        module = BczModule.create(**config["model"])
