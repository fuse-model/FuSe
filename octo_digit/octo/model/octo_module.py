...
class OctoTransformer(nn.Module):
    ...
    token_sizes: Dict[str, jax.ShapeDtypeStruct]
    ...
    def __call__(
        ...
        for readout_name in readouts:
            ...
            # Add positional embedding
            readout_tokens += self._shape_positional_embedding(
                getattr(self, f"{group_name}_pos_embedding"),
                readout_tokens,
            )
        ...
        prefix_outputs, timestep_outputs = self.BlockTransformer_0(
            all_prefix_groups,
            all_timestep_groups,
            train=train,
            verbose=verbose,
    ...
    def _create_positional_embedding(self, name: str, token_shape: tuple):
        if len(token_shape) == 3:  # for prefixes
            shape = (1, *token_shape[-2:])
        elif (
            len(token_shape) == 4
        ):  # for timesteps, create embedding for max_horizon, then truncate
            shape = (1, self.max_horizon, *token_shape[-2:])
        else:
            raise ValueError(f"Invalid tokens shape: {token_shape}")
    ...
    def __call__(
        ...
        for head_name, head in self.heads.items():
            if head_name != 'gen':
                ...
                # generative head should not be called automatically, but included here for parameter initialization
                head_outputs['gen_dummy'] = head(transformer_outputs, modality_idx=jnp.array(0), train=train)
