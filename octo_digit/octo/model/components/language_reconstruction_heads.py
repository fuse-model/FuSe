...
def cosine_loss(
    ...
    cosine_distance = jnp.mean(
        optax.cosine_distance(true_embedding, pred_embedding, epsilon=0.1)
    )
    mse = mse_loss(pred_embedding, true_embedding)
    return cosine_distance, {"loss": cosine_distance, "mse": mse}
...
def contrastive_loss(
    ...
    assert (
        pred_embedding.shape == true_embedding.shape and pred_embedding.ndim == 2
...
class ConinuousLanguageEmbeddingHead(nn.Module, LanguageReconstructionHead, ABC):
    ...
    def setup(self):
        if self.use_map:
            self.pred_map_head = MAPHead(
                name="pred_map_head", num_readouts=self.n_lang_tokens
            )
            self.true_map_head = MAPHead(
                name="true_map_head", num_readouts=self.n_lang_tokens
            )
        ...
        self.temperature = self.param(
            "temperature", jax.nn.initializers.constant(1.0), (1,)
        )

    def _embed_language_into_latent(
        self,
        features: jax.Array,
        map_head: Union[None, MAPHead],
        projection: nn.Dense,
        train: bool,
    ):
        assert features.ndim in {3, 4}, (
            f"Expected features to have shape (batch_size, window_size, num_tokens, embedding_size) or (b, n_tok, emb_size), "
            f"but got shape {features.shape}"
        )
        if self.use_map:
            embeddings = map_head(features, train=train)
        else:
            embeddings = features.mean(axis=-2)
        if embeddings.ndim == 3:
            embeddings = embeddings[:, -1]  # remove window dimension if it exists
    ...
    def __call__(
        ...
        if self.use_map:
            embeddings = self.pred_map_head(features, train=train)[
                :, :, 0
...
class BCZLanguageHead(ConinuousLanguageEmbeddingHead):
    def loss(
        ...
        pred_embeddings_latent, true_embeddings_latent = self(
            transformer_outputs, train=train
        )
...
class CLIPContrastiveHead(ConinuousLanguageEmbeddingHead):
    def loss(
        ...
        loss, metrics = contrastive_loss(
            pred_embeddings_latent, true_embeddings_latent, self.temperature
        )
        return loss, metrics
...
class ContinuousGenerationHead(nn.Module, LanguageGenerationHead, ABC):
    ...
    def setup(self):
        if self.use_map:
            self.map_head = MAPHead(
                name="decode_map_head", num_readouts=self.n_lang_tokens
            )
    ...
    def __call__(
        ...
        embeddings = embeddings[
            :, -1
        ]  # remove window dimension -> (b, n_lang, emb_size)

        pred_lang = self.latent_to_vocab(
            embeddings
        )  # (b, n_lang, vocab_size) probability distributions
        return pred_lang

    def reconstruct_lang(
        self, transformer_outs: Dict[str, TokenGroup], train: bool = True
    ):
...
class SingleHeadContinuousGenerationHead(nn.Module, LanguageGenerationHead, ABC):
    ...
    def setup(self):
        ...
        self.latent_to_vocab = nn.Dense(self.vocab_size, name="latent_to_vocab")
        self.visual_token = self.param(
            f"viz_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )
        self.tac_token = self.param(
            f"tac_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )
        ...
        self.viztac_token = self.param(
            f"viztac_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )
        self.vizaudio_token = self.param(
            f"vizaudio_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )
        self.tacaudio_token = self.param(
            f"tacaudio_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )
        self.viztacaudio_token = self.param(
            f"viztacaudio_class_token",
            nn.initializers.normal(stddev=0.02),
            (self.token_embedding_shape,),
        )

    def get_class_token(self, mode: str):
        if mode == "visual":
            class_token = self.visual_token
        elif mode == "tactile":
            class_token = self.tac_token
        elif mode == "audio":
            class_token = self.audio_token
        elif mode == "visual,tactile":
            class_token = self.viztac_token
        elif mode == "visual,audio":
            class_token = self.vizaudio_token
        elif mode == "tactile,audio":
            class_token = self.tacaudio_token
        elif mode == "visual,tactile,audio":
            class_token = self.viztacaudio_token
        else:
            raise ValueError(mode)
        return class_token

    def __call__(
        self, transformer_outputs: Dict[str, TokenGroup], mode: str, train: bool = True
    ) -> jax.Array:
        token_group = transformer_outputs[self.readout_key]
        ...
        class_token = self.get_class_token(mode)
    ...
    def reconstruct_lang(
        self, transformer_outs: Dict[str, TokenGroup], mode: str, train: bool = True
    ):
        logits = self(transformer_outs, mode=mode, train=train)
        vocab_ids = jnp.argmax(logits, axis=-1)
        # decode_ids = rearrange(vocab_ids, '( b nlang) -> b nlang', nlang=self.n_lang_tokens)
        return vocab_ids
    ...
    def loss(
        ...
        pred_lang_logits = self(transformer_outputs, mode=mode, train=train)
        logits = rearrange(pred_lang_logits, "b nlang vocab -> (b nlang) vocab")
        targets = rearrange(tokenized_language_target, "b nlang -> (b nlang)")
        sftmax_loss = optax.softmax_cross_entropy_with_integer_labels(logits, targets)
