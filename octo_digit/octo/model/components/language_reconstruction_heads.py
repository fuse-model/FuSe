...
class ConinuousLanguageEmbeddingHead(nn.Module, LanguageReconstructionHead, ABC): 
    ...
    def setup(self):
        if self.use_map:
            self.pred_map_head = MAPHead(name='pred_map_head', num_readouts=self.n_lang_tokens)
            self.true_map_head = MAPHead(name='true_map_head', num_readouts=self.n_lang_tokens)
    ...
    def _embed_language_into_latent(
        self, features: jax.Array, map_head: Union[None, MAPHead], projection: nn.Dense, train: bool
    ): 
...
class SingleHeadContinuousGenerationHead(nn.Module, LanguageGenerationHead, ABC): 
    ...
    def reconstruct_lang(self, transformer_outs: Dict[str, TokenGroup], mode: str, train: bool = True): 
