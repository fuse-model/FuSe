...
class JaxMlpLayer(nn.Module):
    ...
    def setup(self): 
        ...
        self.drop1 = nn.Dropout(rate=drop_probs[0]) if drop_probs[0] > 0 else JaxIdentity()
        self.norm = self.norm_layer(hidden_features) if self.norm_layer is not None else JaxIdentity()
        self.fc2 = linear_layer(out_features, use_bias=bias[1])
        self.drop2 = nn.Dropout(rate=drop_probs[1]) if drop_probs[1] > 0 else JaxIdentity()
...
class JaxBlock(nn.Module):
    ...
    def setup(self): 
        ...
        if self.init_values: 
            raise ValueError
        self.ls1 = JaxLayerScale(self.dim, init_values=self.init_values) if self.init_values else JaxIdentity()
        if self.init_values: 
            raise NotImplementedError
        if self.drop_path > 0: 
            raise NotImplementedError
...
class t3ViT(nn.Module): 
    ...
    def setup(self): 
        ...
        self.pos_drop =  nn.Dropout(rate=self.pos_drop_rate) if self.pos_drop_rate > 0 else JaxIdentity()
