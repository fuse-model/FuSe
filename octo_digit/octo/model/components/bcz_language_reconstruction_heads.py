...
class BczCosineFeaturesLanguageHead(nn.Module, LanguageReconstructionHead):
    ...
    @nn.compact
    def __call__(self, encoded_features: ArrayLike, train: bool = False):
        reconstructed_embedding = nn.Dense(self.language_dim)(encoded_features)
        reconstructed_embedding = jnp.mean(
            reconstructed_embedding, axis=-2
        )  # average over window dimension
