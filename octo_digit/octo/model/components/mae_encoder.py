...
class EarlyCNN(nn.Module):
    ...
    def __call__(self, x):
        x = nn.relu(
            nn.Conv(
                features=self.encoder_dim // 8, kernel_size=(6, 6), strides=4, padding=1
            )(x)
        )
        x = nn.relu(
            nn.Conv(
                features=self.encoder_dim // 4, kernel_size=(6, 6), strides=4, padding=1
            )(x)
        )
...
class MAE(nn.Module):
    ...
    def __call__(self, tactile, train: bool = True):
        ...
        tactile_tokens += self.param(
            "tactile_pos_embedding",
            jax.nn.initializers.xavier_normal(),
            (1, num_tactile_patches, self.dim),
        )
