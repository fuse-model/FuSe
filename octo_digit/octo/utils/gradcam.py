...
def extract_tokens_from_all(
    ...
    if transformer_outputs is None:
        output, _ = model.module.apply(
            {"params": model.params}, dummy_observations, dummy_tasks, dummy_pad_mask
        )
    else:
...
def apply_func(
    ...
    if psuedo_loss_type == "loss":
        ...
        apply_func_out = (
            -1 * action_loss
        )  # want to record activations that have positive effect on DECREASING loss
    elif psuedo_loss_type == "prediction":
        unnormalization_statistics = model.dataset_statistics["action"]
        action_head: ActionHead = bound_module.heads["action"]
...
def normalize_image(img: JaxArray) -> JaxArray:
    ...
    if img_max > 0:
        ...
        logging.warn(
            "Image gradient min == image gradient max. Image gradient is likely 0."
        )
    return img
...
def gradCAM(
    ...
    extracted_token_collection = extract_tokens_from_all(
        [obs_key], model, loss_grad, batch, transformer_outputs=transformer_outputs
    )[obs_key]
    ...
    activation_img, gradient_img = remove_batch_and_window(
        batched_imgs=(activation_img, gradient_img), which_in_window=window_step
    )
    ...
    original_image = batch["observation"][obs_key.replace("obs", "image")][
        0, window_step
    ]
    resize_size = original_image.shape[:2]
    resized_gradcam = tf.image.resize(
        gradcam[..., None], resize_size, method="lanczos3", antialias=True
    )[..., 0]
