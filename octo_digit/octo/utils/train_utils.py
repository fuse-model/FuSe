...
def process_text_fuse(batch: Data, text_processor: TextProcessor) -> Data:
    batch_size = batch['task']['language_instruction'].shape[0]
    batch["task"]["language_instruction"] = text_processor.encode(
        [s.decode("utf-8") for s in batch["task"]["language_instruction"]]
    )
    modal_commands = batch["task"]["modal_commands"]
    modal_commands = rearrange(modal_commands, "b m -> (b m)")
    modal_commands = text_processor.encode([s.decode("utf-8") for s in modal_commands])
    batch["task"]["modal_commands"] = jax.tree_map(
        lambda leaf_arr: rearrange(leaf_arr, "(b m) ... -> b m ...", b=batch_size), modal_commands
    )
...
def tvl_loader(params, restore_path, verbose=False):
