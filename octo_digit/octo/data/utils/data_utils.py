...
def sample_match_keys_uniform(d: dict, key_template: str, verbose: bool = True):
...
class AnnotationSelectionManager: 
    ...
    def create_annotation_key_map(self): 
        ...
        self.rev_key_map = {
            v: k for k, v in self.key_map.items()
        }
        self.dataset_keys = np.array(list(self.rev_key_map.keys()))
        self.dataset_valid_keys = np.array([self.key_map[key] for key in self.valid_keys])
        self.dataset_remove_keys = set([self.key_map[key] for key in self.remove_keys])
        self.key_to_index = {
            desc_key: i for i, desc_key in enumerate(self.keys)
        }
        self.index_to_key = {
            i: desc_key for i, desc_key in enumerate(self.keys)
        }
    ...
    def calculate_probabilities(self): 
        ...
        if self.force_uniform_overall: 
            probs = np.array([float(key not in self.remove_keys) for key in self.key_map])
        ...
        self.reconstruction_probabilities = np.array([float(key not in self.remove_keys) for key in self.key_map])
        self.reconstruction_probabilities = normalize(self.reconstruction_probabilities)


        self.probabilities = np.zeros(len(self.key_map),)
        prob_dict = {}
        ...
        if self.uniform_modality_dropout: 
            annotation_probabilities = np.array([float(key in modality_keys) for key in self.key_map])
        else: 
            all_modality_names = set(chain.from_iterable([key.split(',') for key in modality_keys]))
            probabilities =  [   
                reduce(
                    mul, 
                    [self.modality_keep_probabilities[modality] if modality in modalities_tuple else 1 - self.modality_keep_probabilities[modality] for modality in all_modality_names]
                )
                for modalities_tuple in [key.split(',') for key in modality_keys]
            ]
            annotation_probabilities = np.zeros(len(self.key_map),)
            for key, prob in zip(modality_keys, probabilities): 
                annotation_probabilities[self.key_to_index[key]] = prob
            

        annotation_probabilities = normalize(annotation_probabilities, target_sum=1.0 - self.simple_ratio)
        modality_probabilities = {
            modality_key: annotation_probabilities[self.key_to_index[modality_key]] for modality_key in modality_keys
        }
    ...
    def choose_random_keys(self, shape=None, reconstruction=False): 
        probabilities = self.reconstruction_probabilities if reconstruction else self.probabilities
    ...
    def choose_rephrase_keys(self, shape=None): 
        ...
        modality_indices = np.random.choice(len(self.dataset_keys), size=shape, p=self.probabilities)
        rephrase_keys = [f'rephrased_{modality_idx}_{rephrase_idx}' for rephrase_idx, modality_idx in zip(rephrase_indices, modality_indices)]
    ...
    def choose_all_rephrase_keys(self, shape=None): 
        rephrase_indices = np.random.randint(0, self.num_gpt_gen, size=shape + (len(self.dataset_keys),))
