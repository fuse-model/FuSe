...
def apply_trajectory_transforms(
    ...
    *,
    ...
    fuse_augment_strategy: Optional[str] = None,
    ...
    if fuse_augment_strategy is not None:
        dataset = dataset.traj_map(
            partial(
                getattr(task_augmentation, fuse_augment_strategy),
                **fuse_augment_kwargs,
            ),
            num_parallel_calls,
        )
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def _restructure(traj):
        ...
        for new, old in sensor_obs_keys.items():
            if old is None:
                new_obs[new] = tf.repeat("", traj_len)  # padding
            else:
                new_obs[new] = old_obs[old]
        ...
        if "mic" or "mel_spectro" in sensor_obs_keys:
            mic_metadata = {"has_mel_spectro": tf.repeat(traj['traj_metadata']['episode_metadata']['has_mic'], traj_len)}
        else:
            mic_metadata = {}
        traj = {
            "observation": new_obs,
    ...
    if isinstance(dataset_statistics, str):
        ...
        full_dataset = full_dataset.traj_map(_restructure).filter(is_nonzero_length)
    ...
    dataset = dataset.traj_map(_restructure, num_parallel_calls).filter( 
        is_nonzero_length
    )
