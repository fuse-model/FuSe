...
from octo.utils.fuse_constants import name_to_index_generative
        ...
        try: 
            ...
            while t < FLAGS.num_timesteps:
                if time.time() > last_tstep + FLAGS.config.env_params["move_duration"]:
                    if t - reached_top_tstep > FLAGS.config.compositional.top_cooldown:
                        break
                    z = obs['xyz'][-1][-1]
                    if z < FLAGS.config.compositional.bottom:
                        reached_bottom = True
                    if reached_bottom and z > FLAGS.config.compositional.top:
                    ...
                    for key, val in step_info.items():
                        ...
                        info_str += f'{key}:      {val}\n'
                    if FLAGS.verbose:
                        info_str += f"Current height: {obs['xyz'][-1][-1]}\n"
                    print(info_str, end="" if not FLAGS.verbose else '\n')
                    t += 1
                    if truncated:
                        break
            if FLAGS.debug_env or completed_first_half:
                decoded_language = model.decode_language(
                    obs,
                    modality_idx=name_to_index_generative[FLAGS.config.compositional.modality_to_decode],
                )
                print("Decoded language: " + decoded_language)
                task = model.create_tasks(texts=[decoded_language])
                last_tstep = 0
                t = 0
                while t < FLAGS.num_timesteps:
                    if time.time() > last_tstep + FLAGS.config.env_params["move_duration"]:
