...
def compute_result_logs_dis1(log):
    for key, results in log.items():
        ...
        assert sum(total_dict.values()) == 10, (
            key,
            "\n\n",
            results,
            "\n\n",
            total_dict,
            sum(total_dict.values()),
            sorted(results.keys()),
        )
...
def read_eval_logs(
    ...
    read_log_func=read_log,
    keep_models=None,
    keep_exps=None,
    keep_types=None,
):
    log = defaultdict(dict)
    leaf_log_files = glob.glob(os.path.join(log_dir, *(["*"] * 5), "log.txt"))
...
def create_bar_graph(names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
...
def create_bar_graph_multi_labeled(
    ...
    results,
    title,
    save_name,  # results is type to resdict
):
    plt.figure()
    fig, ax = plt.subplots()
    ...
    for i, ann in enumerate(type_to_display):
        ...
        rects = ax.bar(
            ...
            results[ann]["success"],
            width,
            color=["tab:blue" for _ in range(len(model_names))],
            edgecolor="black",
        )
        rects = ax.bar(
            x + offset,
            results[ann]["find"],
            width,
            bottom=results[ann]["success"],
            color=["tab:orange" for _ in model_names],
            edgecolor="black",
        )
        ax.bar_label(
            rects,
            labels=[type_to_display[ann] for _ in range(len(model_names))],
            rotation=90,
            padding=3,
        )
        multiplier += 1
...
def create_bar_graph_tacs(
    ...
    results,
    title,
    save_name,  # results is type (tac/tac2/tac3) to resdict. Resdict: succ/find arr in order of model names
):
    plt.figure()
    fig, ax = plt.subplots()
    ...
    mode_wise_results = defaultdict(partial(defaultdict, list))
        ...
        for exp_dic in results:  #
            for key in exp_dic:
                ...
                for succ_key in ["success", "find"]:
                    ...
                    mode_wise_results[model][succ_key].append(
                        exp_dic[key]["total"][succ_key]
                    )
    ...
    for i, model in enumerate(model_names):
        ...
        rects = ax.bar(
            x + offset,
            mode_wise_results[model]["success"],
            width,
            color=["tab:blue" for _ in range(3)],
            edgecolor="black",
        )
        rects = ax.bar(
            x + offset,
            mode_wise_results[model]["find"],
            width,
            bottom=mode_wise_results[model]["success"],
            color=["tab:orange" for _ in model_names],
            edgecolor="black",
        )
        ax.bar_label(
            rects, labels=[display_names[i] for _ in range(3)], rotation=90, padding=3
        )
        multiplier += 1
    ...
    ax.set_ylim((0, 12))
    ax.set_xticks(
        x + width, ["Wooden block", "Stuffed animal\nNew annotations", "Stuffed animal"]
    )
    # ax.set_xticks(x + width, model_names)
    ax.set_yticks([i for i in range(11) if i % 2 == 0])
    plt.savefig(save_name)
...
def create_bar_graph_dis1(display_names, model_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    ...
    for typ, disp in display_types.items():
        ...
        ax.bar_label(
            rects,
            labels=[disp for _ in range(len(model_names))],
            rotation=90,
            padding=3,
        )
        multiplier += 1
...
def create_bar_graph_dis2(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    ...
    bar_colors = {
        "simple": {"success": "lightblue", "find": "lightsalmon"},
        "default": {"success": "tab:blue", "find": "tab:orange"},
    }
    width = 0.18
    ...
    for typ, disp in bar_keys.items():
        ...
        grasp_colors = [
            bar_colors.get(typ, bar_colors["default"])["success"]
            for _ in range(len(grasps))
        ]
        find_colors = [
            bar_colors.get(typ, bar_colors["default"])["find"]
            for _ in range(len(grasps))
        ]
        ...
        ax.bar_label(
            rects, labels=[disp for _ in range(len(grasps))], rotation=90, padding=3
        )
        multiplier += 1
...
def create_bar_graph_sound1(
    ...
    # ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.09))
    ax.set_ylim((0, 12))
    ax.set_xticks(x + width, model_display_names)
    # ax.axhline(10, linestyle='--')
    # ax.set_xticks(x + width, model_names)
    ax.set_yticks([i for i in range(11) if i % 2 == 0])
    plt.savefig(save_name)
...
def create_bar_graph_dis3(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
...
def create_bar_graph_tac_test(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
...
def create_bar_graph_batch_test(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
...
def plot_tacs(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    ...
    run_names = ["multi_base_20k_20240806_224758", "multi_cont_20k_20240806_072717"]
    simple_display_names = ["Base", "CLIP"]

    create_bar_graph_tacs(
        simple_display_names,
        run_names,
        result_modes,
        "Tactile prompting - test objects",
        "figs/tactiles.png",
    )
    ...
    mod_types = ["tactile"]
    res_dict = {
        key: get_result_dict([(*run_name, keep_mode, key) for run_name in run_names])
    ...
    create_bar_graph(
        simple_display_names,
        res_dict["tactile"],
        "Tactile commands, targeting wooden block",
        f"figs/{keep_mode}.png",
    )


def plot_dis1(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        log_dir, read_log_func=read_log_dis1, keep_models=keep_keys, keep_exps={"dis1"}
    )
    ...
    ball_results = {k[0]: v["total"] for k, v in ball_logs.items()}
    model_names = [
        "multi_base_20k_20240806_224758",
        "multi_cont_20k_20240806_072717",
    ]
    simple_display_names = ["base", "CLIP"]
    create_bar_graph_dis1(
        simple_display_names,
        model_names,
        ball_results,
        "Visual/Tactile disambiguation: targeting foam ball",
        "figs/dis1_ball.png",
    )

    block_logs = filter_dict(logs, lambda k, v: key_field(k, "command_type") == "block")
    block_logs = compute_result_logs_dis1(block_logs)
    block_results = {k[0]: v["total"] for k, v in block_logs.items()}
    create_bar_graph_dis1(
        simple_display_names,
        model_names,
        block_results,
        "Visual/Tactile disambiguation: targeting wood block",
        "figs/dis1_block.png",
    )


def plot_dis2(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        log_dir, read_log_func=read_log_dis1, keep_models=keep_keys, keep_exps={"dis2"}
    )
    ...
    for obj in objects:
        ...
        res.update(logs[(*experiment_name, f"{obj}_viztac")]["total"])
        RESULTS[obj] = res
...
def plot_sound1(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = dict(
        read_eval_logs(
            log_dir,
            read_log_func=read_log_dis1,
            keep_models=keep_keys,
            keep_exps={"sound1"},
        )
    )
    ...
    create_bar_graph_sound1(
        ["no mic", "mic"],
        keep_keys,
        RESULTS,
        "Sound prompting - test keys",
        "figs/sound1_keys.png",
    )
    ...
    create_bar_graph_sound1(
        ...
        keep_keys,
        RESULTS,
        "Sound prompting - test duck",
        "figs/sound1_duck.png",
    )


def plot_dis3(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        log_dir, read_log_func=read_log_dis3, keep_models=keep_keys, keep_exps={"dis3"}
    )
    ...
    for obj in objects:
        ...
        res.update(logs[(*experiment_name, f"{obj}_vizsound")]["total"])
        RESULTS[obj] = res
...
def plot_tac_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        ...
        keep_models=keep_keys,
        keep_exps={"test_tac"},
        keep_types={"yellow_ball_simple", "yellow_ball_tac"},
    )
    ...
    full_results = {
        display_name: compute_dic(model_name)
        for model_name, display_name in display_names.items()
    }

    create_bar_graph_tac_test(
        display_names.values(),
        full_results,
        "Tactile encoder test - ball",
        "figs/test_tac_ball.png",
    )
...
def plot_pod_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        ...
        keep_models=keep_keys,
        keep_exps={"pod"},
        keep_types={"yellow_ball_simple", "yellow_block_tac", "yellow_block_tacviz"},
    )
    ...
    create_bar_graph_batch_test(
        display_names.values(),
        full_results,
        "Batch size ablation - ball",
        "figs/pods_ball.png",
    )


def plot_gen_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        ...
        keep_models=keep_keys,
        keep_exps={"gen", "pod"},
        keep_types={"red_block_viz", "red_block_tac"},
    )
    compute_result_logs(logs)
    ...
    create_bar_graph_batch_test(
        display_names.values(),
        full_results,
        "Generative test - block",
        "figs/gen_block2.png",
    )


def create_bar_graph_ball_test(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
...
def plot_ball_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    create_bar_graph_ball_test(
        display_names.values(),
        full_results,
        "Generative test - ball",
        "figs/gen_ball5.png",
    )


def create_bar_graph_block_test(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    fig.set_figwidth(15)
...
def plot_block_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
...
def create_bar_graph_rope_test(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    fig.set_figwidth(20)
...
def plot_rope_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        ...
        keep_models=keep_keys,
        keep_exps={"gen"},
        keep_types={"green_rope_tac", "green_rope_viztac"},
    )
    compute_result_logs(logs)
    ...
    def compute_dic(model_name):
        ...
        tac_key = [
            log
            for log in rel_log_keys
            if log[-1].endswith("tac") and not log[-1].endswith("viztac")
        ][0]
        tac_res = logs[tac_key]["total"]
    ...
    create_bar_graph_rope_test(
        display_names.values(),
        full_results,
        "Generative test - rope",
        "figs/gen_rope5.png",
    )


def plot_duck_test(log_dir="/home/josh/octo_digit/eval/eval_logs"):
    def filter_dict(dic, filter_fn):
        filtered_dict = {}
        for k, v in dic.items():
            if filter_fn(k, v):
    ...
    logs = read_eval_logs(
        ...
        keep_models=keep_keys,
        keep_exps={"gen"},
        keep_types={"blue_duck_tac", "blue_duck_viztac"},
    )
    compute_result_logs(logs)
    ...
    create_bar_graph_rope_test(
        display_names.values(),
        full_results,
        "Generative test - duck",
        "figs/gen_duck5.png",
    )


def create_bar_graph_bag_test_1(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    fig.set_figwidth(FIGSIZE)
...
def plot_bag_test_1(
    ...
    logs = read_eval_logs(
        ...
        read_log_func=read_log,
        keep_models=keep_keys,
        keep_exps={"bag_test_1"},
        keep_types={"yellow_tac", "yellow_viztac"},
    )
    compute_result_logs(logs)
    ...
    if save_num == -1:
        ...
        while os.path.exists(f"figs/bag_test_1_vers_{save_num}.png"):
            save_num += 1
    create_bar_graph_bag_test_1(
        display_names.values(),
        full_results,
        "Bag test - stuffed animal",
        f"figs/bag_test_1_vers_{save_num}.png",
    )
...
def plot_bag_test_2(
    ...
    logs = read_eval_logs(
        ...
        read_log_func=read_log,
        keep_models=keep_keys,
        keep_exps={"bag_test_2"},
        keep_types={"yellow_paper_tac", "yellow_paper_viztac"},
    )
    compute_result_logs(logs)
    ...
    create_bar_graph_bag_test_1(
        ...
        full_results,
        "Bag test - paper",
        f"figs/bag_test_2_vers_{save_num}.png",
    )


def create_bar_graph_bag_test_3(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    fig.set_figwidth(FIGSIZE)
    ...
    bar_colors = {"default": {"success": "tab:blue", "find": "tab:orange"}}
    width = 0.18
    x = np.arange(len(results))
    multiplier = 0
...
def plot_bag_test_3(
    ...
    logs = read_eval_logs(
        ...
        read_log_func=read_log,
        keep_models=keep_keys,
        keep_exps={"bag_test_3"},
        keep_types={"red_block_simple"},
    )
    compute_result_logs(logs)
    ...
    create_bar_graph_bag_test_3(
        display_names.values(),
        full_results,
        "Bag test - block",
        f"figs/bag_test_3_vers_{save_num}.png",
    )
...
def plot_bag_test_4(
    ...
    logs = read_eval_logs(
        ...
        read_log_func=read_log,
        keep_models=keep_keys,
        keep_exps={"bag_test_4"},
        keep_types={"blue_duck_simple"},
    )
    compute_result_logs(logs)
    ...
    create_bar_graph_bag_test_3(
        ...
        full_results,
        "Bag test - duck",
        f"figs/bag_test_4_vers_{save_num}.png",
    )


def create_bar_graph_bag_average(display_names, results, title, save_name):
    plt.figure()
    fig, ax = plt.subplots()
    fig.set_figwidth(FIGSIZE)
...
def plot_bag_average(
    ...
    full_results = {
        display_name: compute_dic_tacviz(model_name)
        for model_name, display_name in display_names.items()
    }
    ...
    logs = read_eval_logs(
        log_dir, read_log_func=read_log, keep_models=keep_keys, keep_exps={"bag_test_2"}
    )
    compute_result_logs(logs)
    full_results = {
        display_name: compute_dic_tacviz(model_name, logs=logs)
        for model_name, display_name in display_names.items()
    }
    ...
    logs = read_eval_logs(
        log_dir, read_log_func=read_log, keep_models=keep_keys, keep_exps={"bag_test_3"}
    )
    compute_result_logs(logs)
    full_results = {
        display_name: compute_dic_simple(model_name, logs=logs)
        for model_name, display_name in display_names.items()
    }
    all_results[3] = full_results

    logs = read_eval_logs(
        log_dir, read_log_func=read_log, keep_models=keep_keys, keep_exps={"bag_test_4"}
    )
    compute_result_logs(logs)
    full_results = {
        display_name: compute_dic_simple(model_name, logs=logs)
    ...
    for display in display_names.values():
        ...
        total_totals[display] = {
            "success-average": total_success * 1.0 / 6.0,
            "find-average": total_find * 1.0 / 6.0,
        }
    ...
    create_bar_graph_bag_average(
        display_names.values(),
        total_totals,
        "Bag test - average",
        f"figs/bag_average_vers_{save_num}.png",
    )
