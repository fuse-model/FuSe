...
from eval.envs.gym_wrappers import (
    HistoryWrapper,
...
from eval.envs.widowx_env import (
    convert_obs,
    LostConnection,
    state_to_eep,
    wait_for_obs,
...
flags.DEFINE_list("paths", PATHS, ".")

flags.DEFINE_string("paths_file", None, ".")

flags.DEFINE_bool("compile_early", True, ".")
...
flags.DEFINE_string(
    "ckpt_dir",
    os.path.join(os.environ["HOME"], "tpu_octo_ckpts", "ckpts"),
...
def initialize_widowx_env(FLAGS, STATE, env_params, tvl_embedder):
    ...
    env = ResizeImageWrapperDict(STATE.widowx_pure_env, RESIZE_MAP)
    STATE.env = ObsProcessingWrapper(
        env,
        OBS_KEY_MAP,
        CALCULATED_FIELDS,
        flip_channels=FLIP_CHANNELS,
        tvl_embedder=tvl_embedder,
    )
    STATE.pre_history_wrap = STATE.env
    STATE.rewrap = True
    STATE.reset_server = False
    ...
    while not got_response:
        try:
            ...
            should_quit = click_exit_wrapper(
                click.confirm, is_exit_checker=True, text="Quit?", default=False
            )
...
def change_to_model(STATE, new_idx):
    ...
    (
        STATE.curr_name,
        STATE.curr_ckpt_step,
        STATE.model,
        STATE.policy_fn,
    ) = STATE.info_models[STATE.model_idx]
...
def main(_):
    ...
    if FLAGS.paths_file is not None:
        with open(FLAGS.paths_file, "r") as file:
            ...
            if FLAGS.paths[0].find(" ") < 0:
                ...
                FLAGS.paths = [
                    [s.strip() for s in path.split(" ")] for path in FLAGS.paths
                ]
    models = []
    ...
    def calculate_head_params(model):
        ...
        gen_head_params = {
            k: v for k, v in flat_params.items() if k[0].startswith("heads_gen")
        }
        total_size = 0
    ...
    policy_fns = [
        partial(sample_actions, model, rng=jax.random.PRNGKey(0))
        for _, _, model in models
    ]

    def test_with_example_batch(
        model: Model, name: str, policy_function, num_samples=20
    ):
        observations = jax.tree_map(
            lambda arr: arr[0], model.example_batch["observation"]
        )
    ...
    STATE.info_models = [
        (name, ckpt_step, model, policy_fn)
        for (name, ckpt_step, model), policy_fn in zip(models, policy_fns)
    ]
            ...
            for name, ckpt_step, model, policy_fn in STATE.info_models:
                ...
                if new_window_size != prev_window_size:
                    ...
                    for k, v in dummy_obs.items():
                        ...
                        no_window_v = v[0]
                        obs[k] = np.stack(
                            [no_window_v.copy() for _ in range(new_window_size)]
                        )
                else:
                    obs = dummy_obs
        ...
        while True:
            ...
            try:
                ...
                if modality == "g":
                    ...
                    response, should_exit = click_exit_wrapper(
                        click.confirm,
                        text=f"Change run index? currently {STATE.run_idx}:      ",
                        default=STATE.is_first_run,
                    )
                    ...
                    if should_exit:
                        ...
                        if response == "" or response.lower() == "n":
                            ...
                            try:
                                ...
                                print(
                                    f"Invalid response. Continuing with run index = {STATE.run_idx}"
                                )
                    print("#############################")
                    print("Current info:   ")
                    ...
                    if isinstance(STATE.command, str) or STATE.command.is_last():
                        ...
                        next_model_name, next_ckpt_step = STATE.info_models[
                            next_model_idx
                        ][:2]
                    ...
                    if should_exit:
                        ...
                        if should_exit:
                            ...
                            if should_exit:
                                ...
                                for i, (name, ckpt_num, _, _) in enumerate(
                                    STATE.info_models
                                ):
                                ...
                                if response == "" or response.lower() == "n":
                                    new_model_idx = (STATE.model_idx + 1) % len(
                                        STATE.info_models
                                    )
                                else:
                                    try:
                                        ...
                                        print(
                                            f"Received error {e} as a result of entering {response}. Keeping index the same..."
                                        )
                            ...
                            if isinstance(STATE.command, MultimodalCommand):
                                info_str = ", ".join(
                                    [
                                        f"{i}={key}"
                                        for i, key in enumerate(STATE.command.key_list)
                                    ]
                                )
                                while True:
                                    command_idx = input(
                ...
                while t < FLAGS.num_timesteps:
                    if time.time() > last_tstep + STEP_DURATION:
                        ...
                        pred_action = np.array(
                            STATE.policy_fn(obs, STATE.task), dtype=np.float64
                        )
            ...
            should_log, should_exit = click_exit_wrapper(
                click.confirm, text="Log results?   ", default=STATE.should_log
            )
            if not should_log:
                STATE.go_next = False
            ...
            try:
                if should_log:
                    ...
                    if run_result != "exit":
                        ...
                        for i, (im_w, im_l, im_r) in enumerate(
                            zip(wrist_images, dig_left_images, dig_right_images)
                        ):
                        ...
                        video = np.concatenate(
                            [
                                np.stack(wrist_images),
                                np.stack(images),
                                np.stack(dig_left_images),
                                np.stack(dig_right_images),
                            ],
                            axis=1,
                        )
                        imageio.mimsave(
                        ...
                        should_show, should_exit = click_exit_wrapper(
                            click.confirm, text="Log images?   ", default=False
                        )
                        if should_show:
                            overhead = cv2.resize(images[-1], (640, 480))
                            Image.fromarray(overhead).save(
                                os.path.join(video_dir, f"overhead.png")
                            )
                            wrist = cv2.resize(wrist_images[-1], (320, 240))
                            Image.fromarray(wrist).save(
                                os.path.join(video_dir, f"wrist.png")
                            )
                            dig_left = cv2.resize(dig_left_images[-1], (320, 240))
                            Image.fromarray(dig_left).save(
                                os.path.join(video_dir, f"dig_l.png")
                            )
                            dig_right = cv2.resize(dig_right_images[-1], (320, 240))
                            Image.fromarray(dig_right).save(
                                os.path.join(video_dir, f"dig_r.png")
                            )
