...
class MultimodalCommand: 
    ...
    def __post_init__(self): 
        ...
        self.all_annotations = [construct_annotation(key) for key in self.key_list if key not in REMOVE_KEYS]
...
class AnnotationList: 
    ...
    def __post_init__(self): 
        ...
        if len(self.annotations) != len(self.keys): 
            raise ValueError(f'annotations and keys must have save lengths; received {self.annotations} and {self.keys}\nwith lengths {len(self.annotations)} and {len(self.keys)}')
    @property 
    def curr_key(self):
...
DISAMBIG_CMD = AnnotationList(
    annotations=[yball_viztac, yblock_viztac], 
    keys=['ball', 'block']
)
...
DISAMBIG2_CMD = AnnotationList(
    annotations=[
        blue_duck_viztac, blue_duck_simple,
        green_duck_viztac, green_duck_simple, 
        blue_animal_viztac, blue_animal_simple, 
        green_animal_viztac, green_animal_simple
    ], 
    keys=[
        'blue_duck_viztac', 'blue_duck_simple',
        'green_duck_viztac', 'green_duck_simple',
...
DISAMBIG3_CMD = AnnotationList(
    annotations=[
        blue_duck_simple, blue_duck_vizsound, 
        green_duck_simple, green_duck_vizsound
    ], 
    keys=[
        'blue_duck_simple', 'blue_duck_vizsound', 
        'green_duck_simple', 'green_duck_vizsound'
...
TAC_TEST = AnnotationList(
    annotations=[
        yellow_ball_simple, yellow_ball_tac,
        red_block_simple, red_block_tac
    ], 
    keys=[
        'yellow_ball_simple', 'yellow_ball_tac',
        'red_block_simple', 'red_block_tac'
...
final_bag_objects = ['yellow block', 'green cloth', 'brown animal', 'green sponge', 'blue duck', 'blue block', 'yellow sponge', 'green animal', 'multicolored rope', 'silver keys']
final_bag_annotations = [f'Grab the {obj}.' for obj in final_bag_objects]
train_or_test = ['test' if i < 5 else 'train' for i in range(10)]
final_bag_labels = [f'{i}_{train_v_test}_{"_".join(desc.split(" "))}' for i, (train_v_test, desc) in enumerate(zip(train_or_test, final_bag_objects))]
FINAL_BAG_CMDS = [
    AnnotationList(
...
def ann_dict_to_cmds(ann_dict): 
    ...
    out['visual,tactile'] = f'{prefix} looks {ann_dict["visual"]} and feels {ann_dict["tactile"]}.'
    return out
...
final_flat_objects = ['yellow ball', 'red duck', 'blue paper', 'white rope', 'green pepper', 'blue bottle', 'brown shell', 'blue cloth', 'red paper', 'green ball']
final_flat_annotations = [f'Grab the {obj}.' for obj in final_flat_objects]
final_flat_labels = [f'{i}_{train_v_test}_{desc}' for i, (train_v_test, desc) in enumerate(zip(train_or_test, final_flat_objects))]
FINAL_FLAT_PAIRS = {
    label: AnnotationList(
        annotations=[annotation],
        keys=[label]
...
annotation_keys = ['', 'visual', 'tactile', 'audio', 'visual,tactile', 'visual,audio', 'tactile,audio', 'visual,tactile,audio', 'simple']
...
color_annotations = {
    color: f'{push_prefix} {color} button.' for color in sounds
}

color_sound_annotations = {
    color: f'{push_prefix} {color} {button_midfix} {sound}.' for color, sound in sounds.items()
}
...
info_to_command = {
    ...
    ('t2', 't2'): AnnotationList(
        annotations=['Grab the yellow block.'], 
        keys=['simple']
    ),
    ...
    ('rephrase', 'rephrase'):  AnnotationList(
        ...
        keys=['red_block_viz', 'red_block_tac', 'yellow_ball_simple', 'yellow_block_viz', 'yellow_block_tac', 'yellow_block_tacviz']
    ),
    ...
    # ),
    ('gen', 'gen'): AnnotationList(
        annotations=['Grab the object that looks green and feels corded.', 'Grab the object that feels corded.'],
    ...
    ('flat_test_1', ''): AnnotationList(
        annotations=['Grab the object that feels soft.', 'Grab the object that looks blue and feels soft.'],
        keys=['blue_tac', 'blue_viztac']
    ), 
    
    ('bag_test_1', ''): AnnotationList(
        annotations=['Grab the object that feels soft.', 'Grab the object that looks yellow and feels soft.'],
        keys=['yellow_tac', 'yellow_viztac']
    ), 
    
    ('bag_test_2', ''): AnnotationList(
        annotations=['Grab the yellow paper.', 'Grab the object that looks yellow and feels crinkly.'],
        keys=['yellow_paper_tac', 'yellow_paper_viztac']
    ), 
    
    ('bag_test_3', ''): AnnotationList(
        annotations=['Grab the red block.'],
        keys=['red_block_simple']
    ), 
    
    ('bag_test_4', ''): AnnotationList(
        annotations=['Grab the blue duck.'],
        keys=['blue_duck_simple']
    ), 
    ...
    ('platter_cool', ''): AnnotationList(
        annotations=['Grab the object that looks yellow and feels squishy', 'Grab the object that looks yellow and feels crinkly', 'Grab the object that looks blue and feels crinkly'],
...
NAME_TO_COMMAND = lambda mode, name: info_to_command[(mode, '')] # just based on mode flag 
