    ...
    while STATE.reset_environment:
        ...
        while True:
            ...
            try:
                ...
                reached_top_tstep = float("inf")
                completed_first_half = False
                while t < FLAGS.num_timesteps:
                    if time.time() > last_tstep + STEP_DURATION:
                        if t - reached_top_tstep > TOP_COOLDOWN:
                            break
                        z = obs["xyz"][-1][-1]
                        if z < BOTTOM:
                            reached_bottom = True
                        if reached_bottom and z > TOP:
                            completed_first_half = True
                            reached_top_tstep = t
                            reached_bottom = False
                        ...
                        if truncated:
                            break
                if completed_first_half:
                    out = STATE.model.decode_language(obs)
                    gen_str = "|".join([v[0] for k, v in out.items()])
                    command = gen_str[: gen_str.find("</s>")]
                    print("Reconstructed command:   ", command)
                    # obs, _ = STATE.env.reset()

                    task = STATE.model.create_tasks(texts=[command])
                    last_tstep = 0
                    while t < FLAGS.num_timesteps:
                        if time.time() > last_tstep + STEP_DURATION:
                            z = obs["xyz"][-1][-1]
                            ...
                            pred_action = np.array(
                                STATE.policy_fn(obs, task), dtype=np.float64
                            )
                            pred_acs.append(pred_action[0].copy())
                            action = pred_action
                            step_info["forward_pass_time"] = (
                                time.time() - forward_pass_time
                            )
