...
def main(_):
    ...
    #
    #########
    UNIFIED_LANG = FLAGS.config['unified_lang']
    if FLAGS.config['lang_head']:
        if UNIFIED_LANG:
            config['model']['heads']['language_unified'] = FLAGS.config['lang_head']
        else: 
    ...
    if UNIFIED_LANG: 
        effective_weight = FLAGS.config['reconstruction_loss_weight']
    else: 
    ...
    def loss_fn_lang_per_mode(bound_module: OctoModule, batch: Data, train: bool = True, **kwargs): 
    ...
    def loss_fn_lang_unified_lang(bound_module: OctoModule, batch: Data, train: bool = True, **kwargs): 
        total_loss = 0.0
        info = {}
        cached_lang = batch['task'].pop('language_instruction')
        transformer_embeddings_no_lang = bound_module.octo_transformer(
        ...
        batch['task']['language_instruction'] = cached_lang
        true_language_embeddings = bound_module.octo_transformer.embed_language(batch['task'], train=train)
        lang_loss, lang_metrics = bound_module.heads[f"language_unified"].loss(
                transformer_embeddings_no_lang,
                true_language_embeddings,
                batch["observation"]["timestep_pad_mask"],
                train=train,
        ...
        lang_metrics = append_identity_to_metrics(lang_metrics, identity_suffix="unified_lang")      
        total_loss += lang_loss
        info.update(lang_metrics)
        total_loss *= effective_weight
        return total_loss, info

    loss_fn_lang = loss_fn_lang_unified_lang if FLAGS.config['unified_lang'] else loss_fn_lang_per_mode
    ...
    eval_language = FLAGS.config.get('eval_lang', True) and FLAGS.config.get('lang_head', None)
    # Data parallelism
