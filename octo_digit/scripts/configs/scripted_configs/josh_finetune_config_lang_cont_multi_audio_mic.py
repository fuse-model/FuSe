...
def get_config(config_str=None):
    ...
    ANNOTATION_MANAGER_KWARGS = {
        ...
        'reconstruction_loss_keys': ['simple', 'tactile', 'audio', 'visual,tactile',] # ['simple']#  ['simple', 'visual,tactile', 'visual,tactile,audio']
    } 
    ...
    if 'digit_embeddings' in modalities: 
        digit_embedding_keys = {
            'digit_left_embedding': 'digit_0_embedding', 
            'digit_right_embedding': 'digit_1_embedding',
        }
        FINETUNING_KWARGS['sensor_obs_keys'].update(digit_embedding_keys)
        embedding_tokenizer = {"digit_embeddings": ModuleSpec.create( 
                    ProjectionTokenizer,
                    num_output_tokens=50,
                    n_bins=256,
                    obs_keys=[
                        "digit_left_embedding", 
                        "digit_right_embedding"
                    ], 
                )
        }
        NEW_OBS_TOKENIZERS.update(embedding_tokenizer)
    ...
    frame_transform_kwargs = dict(
        resize_size={
            ...
            "digit_left": DIGIT_SIZE, # (256, 256), #(128, 128),
            "digit_right": DIGIT_SIZE, # (256, 256),  # (128, 128)
        },
    ...
    if LANGUAGE_RECONSTRUCTION_HEAD is not None: 
        ...
        # config['update_config']['model']['heads'][f'language_all_lang_0'] = LANGUAGE_RECONSTRUCTION_HEAD
        # config['update_config']['model']['heads'][f'language_all_lang_5'] = LANGUAGE_RECONSTRUCTION_HEAD
        # for i in range(MAX_NUM_ANNOTATION_TYPES): 
        #     config['update_config']['model']['heads'][f'language_all_lang_{i}'] = LANGUAGE_RECONSTRUCTION_HEAD
