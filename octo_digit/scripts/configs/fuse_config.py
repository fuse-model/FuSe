...
from octo.model.components.language_reconstruction_heads import (
    BCZLanguageHead,
    CLIPContrastiveHead,
    ContinuousGenerationHead,
...
from octo.model.components.tvl_vit import tvlViT
from octo.model.components.vit_encoders import ResNet26FILM, SmallStem16
...
def get_config(config_str=None):
    ...
    LANGUAGE_LENGTH = 24
    UNIFIED_LANG = (
        True  # if False, batches during CLIP loss consist of single-type annotations
    )
    ...
    PRETRAINED_LOADERS = [
        ModuleSpec.create(
            tvl_loader,
            restore_path="gs://oier-europe-bucket/ported_weights/tvl/tvl_vitbgs_params_jax.npz",
            verbose=True,
        )
    ]
    DS_NUM = "53.0.0"
    ...
    LANGUAGE_RECONSTRUCTION_HEAD = ModuleSpec.create(
        CLIPContrastiveHead, readout_key="readout_language", use_map=True
    )
    ...
    config = dict(
        ...
        seed=42,
        wandb=dict(project="octo", group=placeholder(str), entity=placeholder(str)),
    ...
    for key, val in BACKGROUND_SUBTRACT_MAP.items():
        ...
        if val.startswith("image_"):
            val = val[len("image_") :]
        frame_transform_kwargs["resize_size"][val] = frame_transform_kwargs[
            "resize_size"
        ][key]
    # If the default data loading speed is too slow, try these:
    config[
