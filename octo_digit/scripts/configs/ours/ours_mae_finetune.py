...
from octo.model.components.tokenizers import BinTokenizer, LowdimObsTokenizer, ImageTokenizer, UnsqueezingImageTokenizer, ProjectionTokenizer, SiglipTokenizer, ImageTokenizerConcatTokens
from octo.model.components.vit_encoders import SmallStem16, ResNet26FILM
from octo.model.components.tokenizers import IdentityObsTokenizer

# from octo.utils.train_utils import resnet_26_loader, tvl_loader
from octo.utils.spec import ModuleSpec
from typing import Iterable
from octo.model.components.language_reconstruction_heads import BCZLanguageHead, CLIPContrastiveHead, ContinuousGenerationHead, SingleHeadContinuousGenerationHead
...
from octo.utils.train_utils import resnet_26_loader, tvl_loader, mae_weights_loader
from octo.model.components.tvl_vit import tvlViT
from octo.model.components.mae_encoder import JaxMAE
...
def get_config(config_str=None):
    ...
    FROZEN_KEYS = ['*hf_model*'] # want to keep newly initialized head unfrozen
    LANGUAGE_LENGTH = 24
    NUM_LANGUAGE_READOUT_TOKENS = 24
    ...
    PRETRAINED_LOADERS = [ModuleSpec.create(mae_weights_loader, restore_path='gs://619c8f721786ba/ported_weights/mae/mae_uniform_95_jax.npz', verbose=True)] 
    DS_NUM = '53.0.0'
    MODALITIES = {'cam_primary', 'cam_wrist', 'vit_left', 'vit_right'}
    ...
    NEW_OBS_TOKENIZERS = {
        # 'uniform_tac': ModuleSpec.create(
        #     IdentityObsTokenizer,
        #     obs_keys=['uniform_tac'],
        # ),
        ...
        "tvl":  ModuleSpec.create(
            ImageTokenizer,
            obs_stack_keys=['image_digit_left', 'image_digit_right'],
            task_stack_keys=[],
            encoder=ModuleSpec.create(JaxMAE, tactile_size=128),
            ),
    }
    ...
    LANGUAGE_RECONSTRUCTION_HEAD = ModuleSpec.create(
        ...
        use_map = True,
        n_lang_tokens=LANGUAGE_LENGTH,
    )
    # LANGUAGE_RECONSTRUCTION_HEAD = None
    GEN_MODES = [('visual',), ('tactile',), ('audio',), ('visual', 'tactile'), ('visual', 'audio'), ('tactile', 'audio'), ('visual', 'tactile', 'audio')]
    LANGUAGE_GENERATION_HEAD = ModuleSpec.create(
        SingleHeadContinuousGenerationHead,
        n_lang_tokens=LANGUAGE_LENGTH,  
    )
    # LANGUAGE_GENERATION_HEAD = None
    MULTI_HEAD = False
    ...
    config = dict(
        gen_modes=GEN_MODES,
        remove_tokenizers = REMOVE_TOKENIZERS,
    ...
    if LANGUAGE_RECONSTRUCTION_HEAD or LANGUAGE_GENERATION_HEAD: 
        config['update_config']['model']['readouts'] = {'language': NUM_LANGUAGE_READOUT_TOKENS}
    config['reconstruction_loss_weight'] = 1.
    config['lang_head'] = LANGUAGE_RECONSTRUCTION_HEAD
    config['gen_head'] = LANGUAGE_GENERATION_HEAD
