...
remap_keys = {"image_0": "image_primary", "image_1": "image_wrist"}
...
for model_name, dataset_version in model_to_dataset.items():
    ...
    builder = tfds.builder_from_directory(
        builder_dir=f"/home/joshwajones/tensorflow_datasets/digit_dataset/{dataset_version}"
    )
    for split in splits:
        for task_mode in task_modes:
            ...
            wrist_imgs_save = [
                cv2.resize(np.array(img), (256, 256)) for img in wrist_images
            ]
            ...
            if verbose:
                ...
                video = np.concatenate(
                    [np.stack(wrist_imgs_save), np.stack(primary_images)], axis=1
                )
            ...
            for step in tqdm.tqdm(range(0, len(primary_images) - WINDOW_SIZE + 1)):
                ...
                true_actions.append(steps[step + 1]["action"])
            ...
            for action_dim, action_label in enumerate(ACTION_DIM_LABELS):
                ...
                axs[action_label].plot(
                    pred_actions[:, 0, action_dim], label="predicted action"
                )
                axs[action_label].plot(
                    true_actions[:, action_dim], label="ground truth"
                )
