...
def main(_):
    ...
    if "/" not in FLAGS.checkpoint_weights_path:
        FLAGS.checkpoint_weights_path = os.path.join(
            "/home/joshwajones/tpu_octo_ckpts", FLAGS.checkpoint_weights_path
        )
    ...
    dataset_statistics = (
        loaded_dataset_stats if FLAGS.is_resnet else model.dataset_statistics
    )
    ...
    policy_fn = partial(sample_actions, model, rng=jax.random.PRNGKey(0))
            ...
            try:
                if FLAGS.video_save_path is not None:
                    if wrist_images and images:
                        ckpt_name = FLAGS.checkpoint_weights_path[
                            FLAGS.checkpoint_weights_path.rfind("/") + 1 :
                        ]
