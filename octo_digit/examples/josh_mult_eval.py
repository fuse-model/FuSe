...
flags.DEFINE_string("ip", "128.32.175.252", "IP address of the robot")
...
def main(_):
    ...
    for name in FLAGS.paths:
        ...
        model = OctoModel.load_pretrained(**load_kwargs)
        print("Model loaded!\n")
    ...
    while STATE.reset_environment:
        ...
        STATE.pre_history_wrap, STATE.env = initialize_widowx_env(
            FLAGS, STATE, env_params, tvl_embedder
        )
        ...
        if FLAGS.compile_early:
            ...
            for name, model, policy_fn in STATE.info_models:
                ...
                dummy_obs["timestep_pad_mask"] = np.array(
                    [[True for _ in range(new_window_size)]]
                )[0]
                recursive_dict_print(dummy_obs)

                dummy_command: Union[str, MultimodalCommand] = NAME_TO_COMMAND(
                    FLAGS.mode, name
                )
            ...
            try:
                ...
                if modality == "g":
                    ...
                    if response:
                        ...
                        STATE.pre_history_wrap, env = initialize_widowx_env(
                            FLAGS, env_params, tvl_embedder
                        )
                    ...
                    response, should_exit = click_exit_wrapper(
                        click.confirm,
                        text=f"Change run index? currently {run_idx}:      ",
                    ...
                    if should_exit:
                        ...
                        if response == "" or response.lower() == "n":
                            ...
                            try:
                                ...
                                print(
                                    f"Invalid response. Continuing with run index = {run_idx}"
                                )
                ...
                while t < FLAGS.num_timesteps:
                    if time.time() > last_tstep + STEP_DURATION:
                        ...
                        if RECUR_PRINT:
                            ...
                            print("########")
                            print(
                                recursive_dict_print(model.example_batch["observation"])
                            )
                        last_tstep = time.time()
                        step_info = {
                            "step": t,
                        ...
                        pred_action = np.array(
                            policy_fn(obs, STATE.task), dtype=np.float64
                        )
            ...
            try:
                if should_log:
                    ...
                    while not got_response:
                        response = (
                            input(
                                "Enter s for success, m for a find, f for failure, or n to avoid logging:     "
                            )
                            .lower()
                            .strip()
                        )
                    ...
                    if run_result != "exit":
                        ...
                        video_dir = os.path.join(
                            model_log_dir, f"{FLAGS.mode}_{run_idx}", curr_time
                        )
                        ...
                        with open(log_file_path, "a") as log_file:
                            ...
                            log_path = os.path.join(
                                FLAGS.eval_log_dir, curr_name, f"eval_{eval_timestamp}"
                            )
