...
class MultimodalCommand: 
    ...
    def __post_init__(self): 
        ...
        self.all_annotations = [construct_annotation(key) for key in string_combos if key not in REMOVE_KEYS]
        self.index = 0 
...
flags.DEFINE_string('ckpt_dir', os.path.join(os.environ['HOME'], 'tpu_octo_ckpts'), 'directory containing the checkpoints')
...
def click_exit_wrapper(click_func, click_kwargs, is_exit_checker=False):
    ...
    while not got_response: 
        try: 
            ...
            response = None 
            should_quit = click_exit_wrapper(click.confirm, {
                "text": "Quit?", 
                "default": False, 
...
def main(_):
    ...
    info_models = [(name, model, policy_fn) for (name, model), policy_fn in zip(models, policy_fns)]
            ...
            try: 
                ...
                if modality == "g":
                    ...
                    if response: 
                        ...
                        if response.lower() == 'n' or response == '': 
                            ...
                            try: 
                                ...
                                print(f'Received error {e} as a result of entering {response}. Incrementing index by one...')
                                model_idx = (model_idx + 1) % len(info_models)
                        
                        response, should_exit = click_exit_wrapper(click.confirm, {"text": "Reset environment?", "default": False})
                        if should_exit: 
                            break 
                        ...
                        command: Union[str, MultimodalCommand] = NAME_TO_COMMAND[curr_name]
                    ...
                    if isinstance(command, MultimodalCommand): 
                        response, should_exit = click_exit_wrapper(click.confirm, {"text": "Proceed to next annotation type?", "default": True})
                        if should_exit: 
                            break 
                        if response is True: 
                            ...
                            while True: 
                                try: 
                                    command_idx = input('Get index (0=viz, 1=tac, 2=viz/tac, 3=viz/aud, 4=tac/aud, 5=viz/tac/aud, or empty input for no change):     ')
                                    if command_idx == '': 
                                        text = command.curr_annotation()
                                        break 
                                    ...
                                    print(f'Received exception {e}')
                                    need_to_exit = click_exit_wrapper(click.confirm, {'text': 'exit?  ', 'default': False})
                                    if need_to_exit:
                                        break 
                ...
                while t < FLAGS.num_timesteps:
                    if time.time() > last_tstep + STEP_DURATION:
                        ...
                        obs['image_digit_left'] = np.array(obs['image_digit_left'], np.int16)
                        obs['image_digit_right'] = np.array(obs['image_digit_left'], np.int16)
                        ...
                        remove_keys = ['task_completed', 'timestep', 'pad_mask_dict', 'timestep_pad_mask']
